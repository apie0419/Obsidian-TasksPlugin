...
export default class FrontmatterKanbanPlugin extends Plugin {
  ...
  async runStartupInitialization(options = {}) {
    ...
    this.startupInitializationPromise = this.initializeStartupResources(options)
      .finally(() => {
        this.startupInitializationPromise = null;
      });
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        if (isFinalAttempt) {
          ...
          new Notice("TaskManagement could not finish startup setup. Check the console for details.");
          return false;
        }
  ...
  async prepareBaseFileForOpen(kind) {
    ...
    try {
      ...
      new Notice(`TaskManagement could not open the ${kind} view. Check the console for details.`);
      return null;
    }
  }
